...
private:
    void rotateFace(int ind){ // To rotate the face clockwise.
        char temp_arr[3][3] = {};
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
        ...
        //Each cell of row 0 is changed to cell of column 0
        for (int i = 0; i < 3; i++) cube[ind][0][i] = temp_arr[2 - i][0];
        //Each cell of column 2 is changed to row 0 cells...and so on...can be seen in physical cube
...
public:
    char cube[6][3][3] {};  //colour of ith Face , jth row , kth col
    ...
    RubiksCube &u() override {
        this->rotateFace(0); //rotating the face clockwise
        //adjusting the edges on other side now.
        char temp_arr[3] = {};
        //saving the back side 2 egde colours in temp
        for (int i = 0; i < 3; i++) temp_arr[i] = cube[4][0][2 - i];
        //now in clockwise sense updating the colours of the edges...
    ...
    //It is used for equality comparison.
    // You can use it to determine if two cubes are in the same configuration.
    ...
    //This overload allows you to copy the state of one RubiksCube3d object into another.
    //useful when you want to create a copy of a cube
...
// convert the entire state of the Rubik's Cube into a unique hash value
struct Hash3d {
